...
namespace ndn::chunks
{
    ...
    usage(std::ostream &os, std::string_view programName, const po::options_description &desc)
    {
        os << "Usage: " << programName << " [options]\n"
           ...
           << "Configuration is read from specified config file\n"
           << "\n"
           << "Options:\n"
           << "  --help, -h                   Print this help message and exit\n"
           << "  --version, -V                Print program version and exit\n"
           << "  --prefix, -p <prefix>        NDN name prefix for the aggregator\n"
           << "  --config, -c <file>          Configuration file path (aggregatorput.ini)\n"
           << "\n"
    ...
    main(int argc, char *argv[])
    {
        ...
        std::string prefix, configFile, signingStr, logFile, logLevel;

        po::options_description visibleDesc("Options");
        visibleDesc.add_options()
            ("help,h", "print this help message and exit")
            ("prefix,p", po::value<std::string>(&prefix)->required(), "NDN name prefix for the aggregator")
            ("config,c", po::value<std::string>(&configFile), "configuration file path (aggregatorput.ini)");
        ...
        spdlog::debug("Reading configuration from: {}", configFile);
        
        // Read configuration from file
        if (!readConfigFile(configFile, opts, signingStr, logFile, logLevel))
        {
            return 2;
        }
